function [ Frame ] = pointclouds(label,numObj,sortedrgb,imgs,imgsd,cams, cam2toW)
...
for currentFrame = 1:size(imgs,3)
    ...
    for j = 1:numObj(currentFrame)                                              %iterate trough objects list
        ...
        if norm(obj)~=0
            ...
            % TRY: Transform object PC to camera 1 reference (NEEDS A
            % DIFFERENT POINTCLOUD.m)
            xyz21=xyz1*cam2toW.R+ones(length(xyz1),1)*cam2toW.T';
